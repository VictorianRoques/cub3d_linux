...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/10/30 21:33:33 by viroques          #+#    #+#             */
/*   Updated: 2020/11/01 18:18:31 by viroques         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/cub3d.h"

static void		ray_pos_box(t_vecteur *vec, int x, t_player *player, t_mlx *mlx)
{
	...
	else
	{
		if (vec->ray_dir_x == 0)
			...
			vec->delta_dist_x = fabs(1 / vec->ray_dir_x);
	}
	if (vec->ray_dir_x == 0)
		vec->delta_dist_y = 0;
	else
	{
		if (vec->ray_dir_y == 0)
			...
			vec->delta_dist_y = fabs(1 / vec->ray_dir_y);
	}
}
...
static void		ft_calcul_step(t_vecteur *vec, t_player *player)
{
	...
	{
		vec->step_x = 1;
		vec->side_dist_x = (vec->map_x + 1.0 -
		player->pos_x) * vec->delta_dist_x;
	}
	if (vec->ray_dir_y < 0)
	{
		vec->step_y = -1;
	...
	{
		vec->step_y = 1;
		vec->side_dist_y = (vec->map_y + 1.0
		- player->pos_y) * vec->delta_dist_y;
	}
}
...
static void		ft_perform_dda(t_mlx *mlx, t_vecteur *vec, t_player *player)
{
	...
	if (vec->side <= 1)
		vec->perp_wall_dist = (vec->map_x - player->pos_x
		+ (1 - vec->step_x) / 2) / vec->ray_dir_x;
	else
		vec->perp_wall_dist = (vec->map_y - player->pos_y
		+ (1 - vec->step_y) / 2) / vec->ray_dir_y;
}
